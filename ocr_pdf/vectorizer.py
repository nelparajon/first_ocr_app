    ...
    def vectorize_doc(self, lemmatized_tokens):
        try:
            if not isinstance(lemmatized_tokens, list) and not all(isinstance(lem, str) for lem in lemmatized_tokens):
                raise TypeError(f"Vectorización. Se esperaba una lista de cadenas, pero se recibió {type(lemmatized_tokens).__name__}")
            ...
            return print(f"TypeError. Vectorización. Error en el tipo de dato: {te}")
    ...
    def similarity_docs(self, docs):
        try:
            if not isinstance(docs, tuple) and not all(isinstance(vector, csr_matrix) for vector in docs):
                raise ValueError(f"Se esperaba una lista de matrices durante el calculo de similitud, pero se recibió {type(docs).__name__}")
